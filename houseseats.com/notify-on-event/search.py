...
## Run the main program in a loop because Github Actions' scheduler can only
## run a job once every 5 minutes and most of the time it is like 10-15 mins
## each interval, which is too long for us.  So we will run this in a loop and
## restart the program every 60 mins.
loop_count = 0
while True:
        ...
        if found_exclude == False:
            ...
            if not found_in_previous_state:
                ...
                if slack_enabled == 'true':
                    send_to_slack(f"""
                        *[Houseseat v2.1]* Search found: <https://lv.houseseats.com/member/tickets/view/?showid={urllib.parse.unquote(event[0])}|{urllib.parse.unquote(event[1])}>  
                    """)
